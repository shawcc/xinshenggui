...
1. 选择视频。
2. 首次使用时填写 OpenAI API Key，或配置兼容的服务地址。
3. 选择中文音色以及清晰分离或快速混音。
4. 点击“自动生成中文配音”。

应用会依次完成语音识别、中文翻译、中文语音合成、背景声处理和 MKV 封装。成品包含原画面、原音轨、原字幕、默认中文配音音轨和自动生成的中文字幕，并自动保存到系统“视频/新声轨”目录。

为完成识别、翻译和语音合成，应用会把压缩后的原片语音发送至用户配置的 OpenAI 或兼容 API；视频画面不会上传。API Key 仅保存在本机应用数据目录。背景声分离、混音和最终封装仍在本机完成。
