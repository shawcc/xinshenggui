...
“清晰分离”使用本地 Demucs 去除原片对白并保留音乐、环境声。桌面安装包会包含该组件，安装包体积约 1GB；“快速混音”不依赖它。
...
生成的 `.dmg` 位于 `dist/`。桌面应用会自动携带 FFmpeg、FFprobe 和本地 Demucs 环境。必须在目标 Mac 上构建，以确保打入对应芯片架构的媒体与 Python 依赖。
...
- 工作流会验证 Demucs 的首次模型下载。用户第一次选择“清晰分离”时，应用会自动下载模型；不需要任何命令或手动配置。
